    ...
    public function getPackageShippingCost(
        ...
        if ($address_id) {
            ...
            $shippingRule = Db::getInstance()->getRow('
                ...
                WHERE sr.id_country = ' . (int)$id_country . '
                AND sr.id_shipping_rule IN (
                    SELECT srp.id_shipping_rule
                    FROM `' . _DB_PREFIX_ . 'shipping_rule_products` srp
                    WHERE srp.id_shipping_rule IN (
                        SELECT srp.id_shipping_rule
                        FROM `' . _DB_PREFIX_ . 'shipping_rule_products` srp
                        GROUP BY srp.id_shipping_rule
                        HAVING COUNT(srp.id_product) = ' . count($productIds) . '
                    )
                    GROUP BY srp.id_shipping_rule
                    HAVING SUM(CASE WHEN srp.id_product IN (' . $productIdsList . ') THEN 1 ELSE 0 END) = ' . count($productIds) . '
                )
            ');
        
            if ($shippingRule) {
                $startRate = (float)$shippingRule['shipping_start_rate'];
                $extraRate = (float)$shippingRule['shipping_extra_rate'];
        
                // Calculate the shipping fee based on total quantity
                $shipping_fee = $startRate + $extraRate * max(0, $totalQuantity - 1);
            }
